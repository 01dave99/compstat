...
Bayesian inference is a statistical method that allows us to estimate the probability of a hypothesis given some observed data. In the context of this report, Bayesian inference is used to better understand the distribution of posterior parameters in a data set. By using Bayesian inference, we can obtain more accurate estimates of the parameters that describe our data, and make more informed decisions about the underlying processes that generated the data. To calculate the parameter posteriors we will sample posterior predictive values with the help of Markov chain Monte Carlo Methods. Additionally, Monte Carlo methods are used to perform a Weibull survival analysis, which allows us to estimate the probability of the survival of a patient. Overall, this report aims to provide a better understanding of how the data was generated, and how the different factors in the data set may be related to the outcome of patient survival time.

\section{Statistical Analysis of Covariates}

In this section we will perform the Bayesian inference on the Covariates.
...
The data seems to follow a counting process so we assume it is poisson distribution $Poi(\lambda)$. Using the non informative Jeffreys prior, $h(\lambda) = \lambda^{-1}$, we can derive that the posterior for the parameter $\lambda$ is Gamma distributed. $\lambda | x \sim Gamma(\alpha=\frac{1}{2} + \sum_{i = 1}^n x_i, \beta = n)$

\subsubsection{1.2 Results}
```{r, echo=FALSE}
new_data = suppressWarnings(as.numeric(mydata$V5))
...
```
The 95% HDI interval calculates to:
```{r, echo=FALSE}
tst = rgamma(1e5, alpha, beta )
hdi(tst)
```

This shows that the expected patient age is 50 years old.
...
The posterior distribution has the following calculated information: 
```{r, echo=FALSE}
...
cat("Centered 95% Confidence Interval: [",cil,",",ciu,"]")
```
With the HPD interval:
```{r, echo = FALSE}
hdi(rbeta(1e5, 1+s, 1 + n - s))
```


\subsection{4. Hepatomegaly}
```{r, include=FALSE}
...
\subsubsection{4.1 Model selection}
...
\subsection{6. Edema}
The presence of edema is coded into (0, 0.5, 1). Where 0 is the class corresponding to no edema and no diuretic therapy for edema. The class 0.5 corresponds to edema present without diuretics, or edema resolved by diuretics. Lastly the class 1 corresponds to edema despite diuretic therapy.


```{r, echo=FALSE}
factors <- factor(mydata$V10,levels=c(0,0.5,1),labels=c("0","0.5","1"))
summary(factors)
```

We observe from the frequencies that most of the patients don't have edema and do not go to diuretic therapy for edema.


\subsection{7. Bilirubin}
...
\subsection{8. Cholesterol}


\subsubsection{8.1 Model selection}
We assume that the data is sampled from a poisson, $Poi(\lambda)$, distribution, and we use the non informative Jeffreys prior for the rate parameter.


\subsubsection{8.2 Results}
```{r, echo=FALSE}
new_data = suppressWarnings(as.numeric(mydata$V12))
...
```{r, echo=FALSE}
new_data = suppressWarnings(as.numeric(mydata$V14))
...
\subsection{12 SGOT}
Data provided in U/ml
\subsubsection{12.1 Model selection}

```{r, echo=FALSE, fig.align = 'center'}
hist(mydata$V16, main="Histogram of SGOT")
```
By inspecting the data and the histogram plot we can make the assumption that the data is sampled from a Gamma distribution with parameters a and b. Using MCMC methods included in the OpenBUGS library we can estimate the posterior parameters. We assume prior parameters $a=b = 0.001$
...
with the following HDI for the parameters a and b respectively:
```{r}
hdi(fit1$sims.list$a)
```
```{r}
...
\subsection{14. Platelets}

Data given in per cubic ml / 1000. 

\subsubsection{14.1 Model selection}

The data resembles a poisson distribution as it is measured in integers. Using the non informative prior we can calculate the posterior gamma distribution for the rate parameter.

\subsubsection{14.2 Results}
```{r, echo=FALSE}
new_data = suppressWarnings(as.numeric(mydata$V18))
...
\subsection{15. Prothrombin}
Prothrombin data given in seconds.

\subsubsection{15.1 Model selection}
...
The 95% HDI for the a - parameter:
```{r, echo=FALSE}
hdi(fit1$sims.list$a)
```
The 95% HDI for the b-parameter:
```{r, echo=FALSE}
hdi(fit1$sims.list$b)
```

\subsection{16. Histologic stage}
...
sum(d)
```
We see that our model did not change much! We continue by neglecting the sex, presence of hepatomegaly, alkaline, triglicerides and platelets:
...
Our model still improved! We stop here and keep that as our survival model. Our remaining covariates are Edema, Bilirubin, Albumin, Urin copper, prothrombin time and histologic stage.
We will give trace plots of the simulated parameters of the model, to show that the values come from the desired stationary distribution:
```{r, echo=FALSE}
...
```
The traceplots look reasonable!
In the after-transplantation lifetime study, those covariates also play the most important role (except urine copper). By sampling from the posterior predictive distribution for the survival time with covariates takes as the means from the liver transplantation study and our mean values in the covariates that are not appearing in the study, i.e taking the covariate vector: $x=[0.5, 17.3, 2.9, 100.77, 14.6, 4].$ we get as a accumulative distribution function:
```{r}
plot(ecdf(fit5$sims.matrix[,9]))
```
which suggests, that the survival probability without liver transplantion is less, when comparing to the liver transplantation paper.
