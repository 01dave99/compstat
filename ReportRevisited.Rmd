...
Before doing our survival analysis we will perform Bayesian inference on the covariates. We will assume sampling distributions and calculate the posterior distributions for the respective parameters using MCMC methods. This will allow us to get a better understanding of the data set.

After analysing the covariats we will apply MCMC methods to perform a Weibull survival analysis and see which parameters are crucial for the patients survival time. We will also compare our results with the after-transplantation lifetime study.
