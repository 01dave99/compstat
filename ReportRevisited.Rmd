...
We will begin this report by making Bayesian inference on the covariates and estimating the posterior distributions for the parameters in the sample distributions. In the end we will make a survival analysis.
...
The data seems to follow a counting process so we assume it is poisson distribution $Poi(\lambda)$. Using the non informative Jeffreys prior, we can derive that the posterior for the parameter $\lambda$ is Gamma distributed.
